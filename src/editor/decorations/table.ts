...
class TableWidget extends WidgetType {
  ...
  eq(other: TableWidget): boolean { return other.html === this.html; }
  toDOM(): HTMLElement {
    ...
    el.querySelectorAll('th').forEach((t) => {
      (t as HTMLElement).style.cssText = 'text-align:left;padding:6px 8px;font-weight:500;border-bottom:1px solid var(--border-color);color:var(--text-primary);';
    });
    el.querySelectorAll('td').forEach((t) => {
      (t as HTMLElement).style.cssText = 'padding:6px 8px;border-bottom:0.5px solid var(--border-light);color:var(--text-secondary);';
    });
    return el;
  }
}
